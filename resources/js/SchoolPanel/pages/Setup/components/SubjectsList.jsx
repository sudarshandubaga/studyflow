...
import { Loader2, Plus, PenBox, Trash2, BookOpen, Search, X, Check, Hash, CheckSquare, Square, Trash } from 'lucide-react';
...
const SubjectsList = () => {
    ...
    const handleDelete = async (id) => {
        if (!window.confirm('Are you sure you want to delete this common subject?')) return;
        try {
    ...
    const handleBulkAction = async (action) => {
        ...
        if (action === 'delete' && !window.confirm(`Delete ${selectedIds.length} subjects forever?`)) return;
        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-100 pb-8">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        <h3 className="font-bold text-2xl text-slate-800 font-outfit tracking-tight">Common Subjects</h3>
                        <p className="text-sm text-slate-400 font-medium">Define subjects available throughout the institution.</p>
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                        ...
                        {isAdding && (
                            <div className="bg-white border-2 border-indigo-600 rounded-[2.5rem] p-6 space-y-5 shadow-2xl shadow-indigo-100 animate-in zoom-in-95 duration-300 relative z-10">
                                <div className="flex items-center justify-between">
                                    <span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">Adding new subject</span>
                                    ...
                                    <div className="relative z-10 space-y-4">
                                        <div className="flex items-start justify-between">
                                            ...
                                            <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-all scale-95 translate-x-2 group-hover:translate-x-0">
                                                <button 
                                                    onClick={(e) => { e.stopPropagation(); setEditingId(subject.id); setEditForm({ ...subject }); }} 
                                        ...
                                        {editingId === subject.id ? (
                                            <div onClick={e => e.stopPropagation()} className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                                                <div className="space-y-3">
                                                    ...
                                                    <label className="flex items-center gap-3 p-3 bg-white rounded-xl border-2 border-slate-100 cursor-pointer">
                                                        ...
                                                        />
                                                        <span className="text-sm font-bold text-slate-700">Active Subject</span>
                                                    </label>
                                                </div>
                                                <div className="flex gap-2">
                                                    <button onClick={() => handleSaveEdit(subject.id)} disabled={saving} className="flex-1 py-3 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest">{saving ? '...' : 'Save'}</button>
                                            ...
                                            <div className="space-y-2">
                                                <div className="space-y-0.5">
                                                    <h4 className="font-bold text-slate-800 font-outfit text-xl tracking-tight line-clamp-1">{subject.name}</h4>
                                                    <p className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em]">{subject.code || 'NO CODE'}</p>
                                                </div>
                                                <div className="pt-2">
                                                    <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${
                                                        subject.is_active ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-400'
                                                    }`}>
                        ...
                        {filtered.length === 0 && !isAdding && (
                            <div className="col-span-full py-24 text-center border-2 border-dashed border-slate-100 rounded-[3rem] bg-slate-50/20">
                                ...
                                <h5 className="font-bold text-slate-700 text-lg uppercase tracking-tight">No academic subjects yet</h5>
